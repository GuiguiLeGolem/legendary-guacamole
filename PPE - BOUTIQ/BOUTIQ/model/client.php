...
class client {
	...
	public function inscriptionClient($nom, $prenom, $email, $motDePasse, $rue, $cp, $ville, $tel) {
		if(strlen($cp) < 11 && strlen($tel) < 11){
			$sql = $this->pdo->query("INSERT INTO client (`nomClient`,`prenomClient`,`emailClient`,`motDePasseClient`,`rueClient`,`cpClient`,`villeClient`,`telClient`) VALUES ('".$nom."','".$prenom."','".$email."','".password_hash($motDePasse, PASSWORD_DEFAULT)."','".$rue."','".$cp."','".$ville."','".$tel."');");
			return true;
		}
		else{
			return false;
